...
class DEAP_Network(OptDevice):
    """Simulation of an array of N all-pass and N add-drop microring resonators """
    ...
    def get_S_params_AP(self): # For all-pass MRR

        self.lam_spacing = np.mean([self.params["MRR_AP"+str(i)][-1] for i in range(self.num_MRR)])
        ...
        #method to define T1,T2,T3
        self.MRR_array_TD = np.array([AddDropMRR(*self.params['MRR_AP'+str(i)]).get_S_params()
        ...
        self.S_params = np.array([self.S11_N])
        
        return self.S_params

    def get_S_params_AD(self): # For all-pass MRR

        self.lam_spacing = np.mean([self.params["MRR_AD"+str(i)][-1] for i in range(self.num_MRR)])
        ...
        self.MRR_array_TD = np.array([AddDropMRR(*self.params['MRR_AD'+str(i)]).get_S_params()
                                      for i in range(self.num_MRR)], dtype=np.complex128)
        #Thru Transmission for N rings
        self.S11_N = 1.0
        # Vectorized computation for the Thru port
        ...
        #r2 values 
        r2_N = [self.params["MRR_AD"+str(i)][4] for i in range(self.num_MRR)]
    ...
    def propagate(self, input_field: complex):
        ...
        E_T_ap = self.get_S_params_AP() * input_field *  1 / np.sqrt(2) 

        # Calculate the output fields [E2,E8] using the MRM_S
        [E_T, E_D] = self.get_S_params_AD() * E_T_ap *  1 / np.sqrt(2) 
    ...
    def __repr__(self):
        ...
        return (f"<MRR: port_T={self.port_T}, port_D={self.port_D}, "
                f"num_MRR={self.num_MRR/2}, Lw={self.Lw}, n_idx={self.n_idx}")
